...
function json_ccp {
    ...
    sed -e "s/\${ORG}/$1/" \
        ...
        -e "s#\${CAPEM}#$CP#" \
        -e "s/\${ORGID}/$6/" \
        organizations/new-ccp-template.json
}
...
function yaml_ccp {
    ...
    sed -e "s/\${ORG}/$1/" \
        ...
        -e "s#\${CAPEM}#$CP#" \
        -e "s/\${ORGID}/$6/" \
        organizations/new-ccp-template.yaml | sed -e $'s/\\\\n/\\\n          /g'
}
...
echo "$(json_ccp $ORG $P0PORT $CAPORT $PEERPEM $CAPEM $ORGID)" > organizations/peerOrganizations/manufacturer.example.com/connection-manufacturer.json
echo "$(yaml_ccp $ORG $P0PORT $CAPORT $PEERPEM $CAPEM $ORGID)" > organizations/peerOrganizations/manufacturer.example.com/connection-manufacturer.yaml
...
echo "$(json_ccp $ORG $P0PORT $CAPORT $PEERPEM $CAPEM $ORGID)" > organizations/peerOrganizations/seller.example.com/connection-seller.json
echo "$(yaml_ccp $ORG $P0PORT $CAPORT $PEERPEM $CAPEM)" > organizations/peerOrganizations/seller.example.com/connection-seller.yaml
